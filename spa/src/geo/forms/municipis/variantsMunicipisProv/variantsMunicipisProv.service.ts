...
export class MunicipisVariantsMunicipisProvFormService {
  private municipisService = inject(MunicipisService);
  ...
  submit(value: any = false, cache = true, only_cache = false): Observable<__model.ConjuntElementsVarMunicipi> {
    if (value === false) {
  ...
  try(
    subject: ReplaySubject<__model.ConjuntElementsVarMunicipi>,
    value: any,
    cache_hit: boolean,
    cache: boolean,
    waitOnRetry = 1000,
    maxRetries = environment.apiRetries,
  ): void {
      ...
      .pipe(
        ...
        catchError((error) => {
          if (error.status >= 500 && maxRetries > 0) {
            ...
            setTimeout(
              () => this.try(subject, value, cache_hit, cache, waitOnRetry + 1000, maxRetries - 1),
              waitOnRetry,
            );
          } else {
